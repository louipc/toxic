...
static Tox *init_tox(void)
{
    ...
    /* Callbacks */
    tox_callback_self_connection_status(m, on_connectionchange, NULL);
    ...
    tox_callback_file_receive(m, on_file_sendrequest, NULL);
    tox_callback_file_control(m, on_file_control, NULL);
    tox_callback_file_receive_chunk(m, on_file_data, NULL);
...
void *thread_cqueue(void *data)
{
    ...
    while (true) {
        ...
        for (i = 2; i < MAX_WINDOWS_NUM; ++i) {
            ...
            if (toxwin != NULL && toxwin->is_chat && tox_friend_get_connection_status(m, toxwin->num, NULL) != TOX_CONNECTION_NONE)
